...
- **Cauchy dispersion model**: `n(λ) = B + C/λ²` — wavelength-dependent refractive index for optical glasses. Preset coefficients for BK7 crown, F2 flint, and ED (extra-low dispersion) glasses: FPL51, FPL53, S-FPL51. Enables chromatic analysis (blue light refracts more than red).
- **Refracting telescope**: Objective lens focuses light directly to focal plane. Zero central obstruction. Lens geometry auto-computed from lensmaker's equation. Supports four objective types:
  - **Singlet**: Single lens element (symmetric biconvex). Shows strong chromatic aberration in polychromatic mode.
  - **Achromatic doublet**: Cemented crown (BK7) + flint (F2) doublet. Uses achromatism condition to minimize chromatic aberration (~10× better than singlet).
  - **APO doublet**: Cemented ED glass (FPL51) + crown (BK7) doublet. Extra-low dispersion glass provides ~2× better correction than standard achromat.
  - **APO triplet**: Three-element design with ED + flint + ED (FPL51 + F2 + FPL51). Excellent chromatic correction across full visible spectrum.
...
- **Apochromatic doublet**: ED glass (FPL51) + crown (BK7) cemented doublet. ED glass has ultra-low dispersion (Abbe number ~81 vs ~64 for BK7), enabling superior chromatic correction. Brings three wavelengths to nearly the same focus. ~2× better than standard achromat. Used in high-end refractors for astrophotography and planetary observation.
- **Apochromatic triplet**: Three-element design: ED + flint + ED (FPL51 + F2 + FPL51). Classic triplet configuration with positive-negative-positive power distribution. Provides excellent chromatic correction across the full visible spectrum. Residual secondary spectrum ~5× smaller than achromat. Ray-traced through all six surfaces (three elements, two cemented interfaces).
